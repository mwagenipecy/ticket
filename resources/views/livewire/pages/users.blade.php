    ...
    <div class="box mt-6">
        <form wire:submit.prevent="createUser">
            <div class="border-b border-slate-200/60 p-5 dark:border-darkmode-400 lg:flex-row">
                ...
                <!-- Name Input -->
                <label for="name" class="inline-block mb-2 mt-2">Name</label>
                ...
                <!-- Email Input -->
                <label for="email" class="inline-block mb-2 mt-2">Email</label>
                ...
                <!-- Level Select -->
                <label for="level" class="mt-2 inline-block mb-2">Select Level</label>
                <select id="level" wire:model="level" class="w-full text-sm border-slate-200 shadow-sm rounded-md text-xs mt-2 inline-block mb-2 group-[.form-inline]:mb-2 group-[.form-inline]:sm:mb-0 group-[.form-inline]:sm:mr-5 group-[.form-inline]:sm:text-right
                    disabled:bg-slate-100 disabled:cursor-not-allowed disabled:dark:bg-darkmode-800/50 [&amp;[readonly]]:bg-slate-100 [&amp;[readonly]]:cursor-not-allowed [&amp;[readonly]]:dark:bg-darkmode-800/50
                    transition duration-200 ease-in-out w-full text-sm border-slate-200 shadow-sm rounded-md py-2 px-3 pr-8 focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus:border-primary focus:border-opacity-40
                    dark:bg-darkmode-800 dark:border-transparent dark:focus:ring-slate-700 dark:focus:ring-opacity-50 group-[.form-inline]:flex-1 text-lg py-1.5 pl-4 pr-8 sm:mr-2 sm:mt-2 sm:mr-2 sm:mt-2">
        ...
        <div class="intro-y col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
            ...
            <div class="mt-3 w-full sm:ml-auto sm:mt-0 sm:w-auto md:ml-0">
                <div class="relative w-56 text-slate-500">
                    <input
                        data-tw-merge
                        type="text" placeholder="Search..." class="disabled:bg-slate-100 disabled:cursor-not-allowed dark:disabled:bg-darkmode-800/50 dark:disabled:border-transparent [&amp;[readonly]]:bg-slate-100 [&amp;[readonly]]:cursor-not-allowed [&amp;[readonly]]:dark:bg-darkmode-800/50 [&amp;[readonly]]:dark:border-transparent transition duration-200 ease-in-out w-full text-sm border-slate-200 shadow-sm rounded-md placeholder:text-slate-400/90 focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus:border-primary focus:border-opacity-40 dark:bg-darkmode-800 dark:border-transparent dark:focus:ring-slate-700 dark:focus:ring-opacity-50 dark:placeholder:text-slate-500/80 group-[.form-inline]:flex-1 group-[.input-group]:rounded-none group-[.input-group]:[&amp;:not(:first-child)]:border-l-transparent group-[.input-group]:first:rounded-l group-[.input-group]:last:rounded-r group-[.input-group]:z-10 !box w-56 pr-10 !box w-56 pr-10"
                    />
                    <i
                        data-tw-merge
                        data-lucide="search"
                        class="stroke-1.5 w-5 h-5 absolute inset-y-0 right-0 my-auto mr-3 h-4 w-4 absolute inset-y-0 right-0 my-auto mr-3 h-4 w-4"
                    ></i>
                ...
                <div class="box w-full " >
                    ...
                    <div class="flex flex-wrap items-center justify-center p-5 lg:flex-nowrap">
                        <div class="mb-4 mr-auto w-full lg:mb-0 lg:w-1/2">
                            @php
                                $total_tasks = \Illuminate\Support\Facades\DB::table('tasks')
                                    ->where('assigned_to_id', $user->id)
                                    ->count();

                                $closed_tasks = \Illuminate\Support\Facades\DB::table('tasks')
                                    ->where('assigned_to_id', $user->id)
                                    ->where('status', 'CLOSED')
                                    ->count();

                                // Avoid division by zero
                                $completion_rate = $total_tasks > 0 ? round(($closed_tasks / $total_tasks) * 100, 2) : 0;
                            @endphp

                            <div class="flex text-xs text-slate-500">
                                <div class="mr-auto">Completion rate</div>
                                <div>{{ $completion_rate }}%</div>
                            </div>

                            <div
                                data-tw-merge
                                class="w-full h-2 bg-slate-200 rounded dark:bg-black/20 mt-2 h-1"
                            >
                                <div
                                    data-tw-merge
                                    role="progressbar"
                                    ...
                                    aria-valuemin="0"
                                    aria-valuemax="100"
                                    class="bg-primary h-full rounded text-xs text-white flex justify-center items-center"
        ...
        <div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-white p-8 rounded-lg shadow-lg">
                ...
                <select id="level" wire:model="level" class="w-full text-sm border-slate-200 shadow-sm rounded-md text-xs mt-2 inline-block mb-2 group-[.form-inline]:mb-2 group-[.form-inline]:sm:mb-0 group-[.form-inline]:sm:mr-5 group-[.form-inline]:sm:text-right
                    ...
                    @foreach(DB::table('levels')->get() as $status)
                        <option value="{{ $status->id }}">{{ $status->level_name }}</option>
                    @endforeach
                </select>
                @error('level') <span class="text-red-500">{{ $message }}</span> @enderror
                <div></div>
